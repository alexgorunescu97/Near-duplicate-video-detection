...
from utils import load_dataset, load_feature_files, global_vector, load_features
...
def triplet_generator_vcdb(dataset, vcdb_features, threshold, train_dev_split):
    """
    ...
    if train_dev_split:
        triplets_train, triplets_dev = train_test_split(triplets, test_size=train_dev_split, random_state=42)
        return triplets_train, triplets_dev
    else:
        return triplets, None


def triplet_generator_cc(dataset, cc_web_features, train_dev_split):
    """
      Function that generates video triplets from CC_WEB_VIDEO.

      Args:
        dataset: dataset object that contains the VCDB video pairs
        cc_web_features: global features of the videos in CC_WEB_VIDEO
      Returns:
        triplets: the list of triplets with video indexes
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--feature_files', type=str, default="",
                        help='Path to the .npy files that contains the feature vectors '
                             ...
                             'by a tab character (\\t)')
    parser.add_argument('-g', '--global_features', type=str, default="", 
                        help="Path to the .npy file containing global features")
    parser.add_argument('-s', '--split', type=float, default=0.0,
    ...
    if args['feature_files'] and args['global_features']:
        raise Exception('Only one of --feature_files and --global_features is required')
    if not args['feature_files'] and not args['global_features']:
        raise Exception('Either one of --feature_files and --global_features must be provided')
    ...
    if args['feature_files']:
        ...
        features = np.load(args['global_features']).astype(np.float32)
    if 'vcdb' in args['dataset'].lower():
        triplets_train, triplets_dev = triplet_generator_vcdb(dataset, features, args['overlap_threshold'], args["split"])
    elif 'cc_web_video' in args['dataset'].lower():
        triplets_train, triplets_dev = triplet_generator_cc(dataset, features, args["split"])
    
    if triplets_dev:
        np.save(os.path.join(args['output_dir'], '{}_triplets_dev'.format(args['dataset'])), triplets_dev)
    np.save(os.path.join(args['output_dir'], '{}_triplets'.format(args['dataset'])), triplets_train)
