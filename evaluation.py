...
if __name__ == '__main__':
    ...
    mAP_dml, pr_curve_dml = evaluate(cc_dataset['ground_truth'], cc_dataset['index'], cc_dataset['queries'], similarities, args["results_path"],
                                     positive_labels=args['positive_labels'], all_videos=False, is_baseline=False)
    mAP_base, pr_curve_base = evaluate(cc_dataset['ground_truth'], cc_dataset['index'], cc_dataset['queries'], baseline_similarities, args["results_path"],
                                       positive_labels=args['positive_labels'], all_videos=False, is_baseline=True)
    print('CC_WEB_VIDEO')
    ...
    mAP_dml, pr_curve_dml = evaluate(cc_dataset['ground_truth'], cc_dataset['index'], cc_dataset['queries'], similarities, args["results_path"],
                                     positive_labels=args['positive_labels'], all_videos=True, is_baseline=False)
    mAP_base, pr_curve_base = evaluate(cc_dataset['ground_truth'], cc_dataset['index'], cc_dataset['queries'], baseline_similarities, args["results_path"],
                                       positive_labels=args['positive_labels'], all_videos=True, is_baseline=True)
    print('\nCC_WEB_VIDEO*')
    print('baseline mAP: ', mAP_base)
    print('DML mAP: ', mAP_dml)
    plot_pr_curve(pr_curve_dml, pr_curve_base, 'CC_WEB_VIDEO*')
