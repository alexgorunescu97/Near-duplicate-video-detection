...
class DNN(object):
    ...
    def __init__(self,
        ...
        if reg == 'l2':
            ...
            self.regularizer = tf.contrib.layers.l1_regularizer(scale=weight_decay) if trainable else None
    ...
    def build(self, hidden_layer_sizes):
        ...
        for i, M in enumerate(hidden_layer_sizes):
            ...
            if i == 0:
                net = tf.contrib.layers.dropout(net, keep_prob=self.dropout_rate_1)
            elif i == 1:
                net = tf.contrib.layers.dropout(net, keep_prob=self.dropout_rate_2)
    ...
    def test(self, X):
        ...
        summary, cost, error = self.sess.run(self.test_op, feed_dict={self.input: X, self.dropout_rate_1: 1, self.dropout_rate_2: 1})
    ...
    def embeddings(self, X):
        ...
        """
        return self.sess.run(self.output, feed_dict={self.input: X, self.dropout_rate_1: 1, self.dropout_rate_2: 1})