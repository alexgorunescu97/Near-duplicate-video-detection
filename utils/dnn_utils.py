...
def evaluate(ground_truth, index, queries, similarities, results_path, positive_labels='ESLMV', all_videos=False, is_baseline=False):
    """
      Function that plots the PR-curve.
    ...
    for query_set, labels in enumerate(ground_truth):
        ...
        s = 0.0
        y_target, y_score = [], []
        n_positive_videos = len([label for label in labels.values() if label in positive_labels])
        for video, sim in similarities[query_set]:
            if video and (all_videos or video in labels):
                y_score += [sim]
                y_target += [0.0]
                ri += 1
                ...
                if i < n_positive_videos and not is_baseline and results_path is not None:
                    ...
                    elif video in labels and labels[video] in positive_labels:
                        results_file.write(f'Positive video ({labels[video]}) labeled correctly ({"CC_WEB_VIDEOS*" if all_videos else "CC_WEB_VIDEOS"}): {index[video]} for query {index[queries[query_set]]}\n')
        ...
        for i in lrange(20, 0, -1):
            ...
            t += [thresholds[idx[np.argmax(precision[idx])]]]
        pr += [p + [1.0]]
        tr += [t + [1.0]]
    if results_path is not None:
        results_file.close()
    return mAP / len(ground_truth), np.mean(pr, axis=0)[::-1], np.mean(tr, axis=0)[::-1]
